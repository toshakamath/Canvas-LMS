...
export default class ProfileTeacherEdit extends Component {
  constructor(props) {
    super(props);
    this.state = {
      userdetails: [],
  ...
  componentDidMount() {
    ...
    //   });
    axios
      .post("http://localhost:3001/teacher/profile/edit", data)
      .then((response) => {
  ...
  onChangeHandler = (e) => {
    console.log("CHANGEEEEEE: "+e.target.name, " : ", e.target.value);
    this.setState({
  ...
  updateProfile = (e) => {
    ...
    //make a post request with the user data
    axios
      .post("http://localhost:3001/teacher/profile/update", data)
      .then((response) => {
        console.log("Status Code : ", response.status);
        console.log("response.data",response.data);
