...
import {setFileUpload, fileUpload, } from "../../actions/FileAction";
...
class FileUpload extends Component {
  ...
  uploadChangeHandler = (e) => {
    ...
    this.props.setFileUpload({
      upload: e.target.files[0]  //name=up
    });
  };
  displayfiles = (e) => {
  ...
  submit = (e) => {
    ...
    console.log("filename:", this.props.filedetails.setfileupload.upload);
    var em = JSON.parse(localStorage.getItem("userdata")).email;
    console.log(em);
    data.append("up", this.props.filedetails.setfileupload.upload);
    data.append("email", em);
    data.append("courseid", courseid);
    console.log("entire data that we are sending to backend",data);
    this.props.fileUpload(data, this.props.history, courseid);
...
export default connect(mapStateToProps, {setFileUpload, fileUpload})(FileUpload);